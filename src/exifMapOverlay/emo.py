...
class EmoSettings():
    ...
    def read_from_file(self):
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName), exist_ok=True)
            with open(self.get_settings_path(), 'w') as f:
                json.dump(self.data, f)
            pass
    
    def get_settings_path(self) -> str:
        return os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName, 'emo_settings.json')
    

class ExifMapOverlay():
    ...
    def print_image(self, lat: float, lon: float, zoom_factor: int, width: int, height: int, url_template: str, tile_size: int) -> str:
        """
        ...
        """
        file_path = self.get_temp_map_name(lat, lon, zoom_factor, width, height)
        if not os.access(os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName), os.R_OK):
            os.mkdir(os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName))
        if not os.access(file_path, os.R_OK):
            m = StaticMap(width, height, url_template=url_template, tile_size=tile_size, delay_between_retries=1, 
                          cache_dir=os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName, 'tiles'))
    ...
    def get_temp_map_name(self, lat, lon, zoom_factor:int, width: int, height: int):
        temp_dir = os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName)
        file_name = f"{lat:.6f}_{lon:.6f}_zoom{zoom_factor}_w{width}_h{height}.png"  # 6 decimally ~ 0.1m
        file_path = os.path.join(temp_dir, file_name)
        return file_path

    def get_name_from_coordinates(self, lat: float, lon: float, result_language: str | None) -> str:
        temp_dir = os.path.join(tempfile.gettempdir(), ExifMapOverlay.AppName)
    ...
    def borderless(self, image_path, place_name, font_size, display_time=None):
        root = tk.Tk()
        try:
    ...
    def main(self):
        ...
        try:
            ...
            return
        png_path = self.print_image(coords[0], coords[1], 
        ...
        name = self.get_name_from_coordinates(coords[0], coords[1], 
                                        settings.data['nominatim_language'])
        self.borderless(png_path, name, settings.data['place_text_font_size'])
